...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # when a ghost is not captured
        if noisyDistance is not None:
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                # unmormalized P(position | noisyDistance) = P(noisyDistance | position) * P(position)
                # = P(noisyDistance | trueDistance) * P(trueDistance | position) * P(position)
                allPossible[p] = self.beliefs[p] * emissionModel[trueDistance]
        # when a ghost is captured
        else:
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        allPossible = util.Counter()
        # B(X_t+1) = sum of P(X_t+1 | X_t) * B(X_t)
        for oldPosm, oldProb in self.beliefs.items():
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPosm))
            for newPos, prob in newPosDist.items():
                allPossible[newPos] += oldProb * prob
        self.beliefs = allPossible
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is not None:
            for p in self.particles:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                # weight(sample x at position p) = P(noisyDistance | position = p)
                # = P(noisyDistance | position p's trueDistance) | P(trueDistance | position p)
                weightedParticles[p] += emissionModel[trueDistance]
            if not any(weightedParticles.values()):
                self.initializeUniformly(gameState)
            else:
                # resample
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] is None:
                    ...
                    prod *= emissionModels[i][trueDistance]
            weightedParticles[particle] += prod
        if not any(weightedParticles.values()):
            self.initializeParticles()
            for particle in self.particles:
                for i in range(self.numGhosts):
                    if not noisyDistances[i]:
                        particle = self.getParticleWithGhostInJail(particle, i)
        else:
            self.particles = [util.sample(weightedParticles) for i in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                # obtain the distributions over new positions for a single ghost, given the list (prevGhostPositions)
                # of previous positions of aLL ghosts
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, list(oldParticle)), i,
                                                             self.ghostAgents[i])
