...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        likelyPositions = [max(d.keys(), key=lambda pos: d[pos]) for d in livingGhostPositionDistributions]
        # get the closest position (regarding maze distance) among the most likely position list
        closestPos = min(likelyPositions, key=lambda pos: self.distancer.getDistance(pos, pacmanPosition))
        # get the action that minimize the maze distance to the closest position
        act = min(legal, key=lambda a:self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, a), closestPos))
        return act
